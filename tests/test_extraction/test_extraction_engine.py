...
class TestExtractionEngine:
    ...
    async def test_openai_extractor_extract_success(self):
        ...
        mock_response = Mock(output_parsed=ClinicalSummary(
    ...
    def test_openai_extractor_init_missing_model(self):
        with pytest.raises(ConfigurationError, match="OPENAI_MODEL_FOR_EXTRACTION not found"):
    ...
    async def test_gemini_extractor_extract_success(self):
        ...
        mock_response = Mock(parsed=ClinicalSummary(
            patient_mood="anxious",
            exercises_completed=["balance exercises"],
            symptoms_mentioned=["anxiety"],
            next_steps="Continue daily exercises",
        ...
        extractor.client.models.generate_content = Mock(return_value=mock_response)
        ...
        extractor.client.models.generate_content.assert_called_once_with(
    ...
    async def test_gemini_extractor_extract_wrong_type(self):
        ...
        extractor.client.models.generate_content = Mock(return_value=bad_response)
    ...
    def test_gemini_extractor_init_missing_model(self):
        with pytest.raises(ConfigurationError, match="GEMINI_MODEL_FOR_EXTRACTION not found"):
    ...
    async def test_deepseek_extractor_extract_success(self):
        ...
        mock_response = Mock(choices=[
            Mock(message=Mock(content=mock_summary.model_dump_json()))
            ])

        extractor = DeepSeekExtractor(api_key="sk_test", model="deepseek-chat", base_url="https://api.deepseek.com")
        extractor.client = AsyncMock()
    ...
    async def test_deepseek_extractor_extract_wrong_type(self):
        extractor = DeepSeekExtractor(api_key="sk_test", model="deepseek-v4-flash", base_url="https://api.deepseek.com")
        extractor.client = AsyncMock()
