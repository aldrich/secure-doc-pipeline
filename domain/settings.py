...
class Settings(BaseSettings):
    ...
    database_url: str = (
        "postgresql+asyncpg://postgres:postgres@localhost:5433/clinical_pipeline"
    )
...
def _validate_engine(
    ...
    if not getattr(settings, model_field):
        raise ConfigurationError(f"{display_name} model is required for {role} engine")
...
def validate_settings(settings: "Settings"):
    ...
    if missing:
        raise ConfigurationError(
            "Required settings are missing or empty: " + ", ".join(missing)
        )
